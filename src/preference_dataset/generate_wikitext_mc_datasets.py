...
def prompt_gpt(client, article: str) -> Optional[MCQ]:
    completion = client.beta.chat.completions.parse(
        model=OPENAI_MODEL,
        messages=[
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": f"Write 10 multiple-choice questions based on the following article.\nARTICLE:\n{article}"},
        ],
...
if __name__ == "__main__":
    ...
    dataset_bio_retain_dpo = {
        "prompt": [],
        "chosen": [],
        "rejected": [],
    }
    
    dataset_cyber_retain_dpo = {
        "prompt": [],
        "chosen": [],
        "rejected": [],
    }
    ...
    for i, sample in enumerate(tqdm(dataset)):
        ...
        try:
            mc_question_list, usage = prompt_gpt(client, article)
        ...
        for mc in mc_question_list.multiple_choice_questions:
            try:
                ...
                dataset_bio_retain_dpo["prompt"].append(question)
                dataset_bio_retain_dpo["chosen"].append(answer)
                dataset_bio_retain_dpo["rejected"].append(random.sample(REFUSAL_LIST,1)[0])
                
                question, answer, explanation = format_question(mc, "cybersecurity")
                dataset_cyber_retain_dpo["prompt"].append(question)
                dataset_cyber_retain_dpo["chosen"].append(answer)
                dataset_cyber_retain_dpo["rejected"].append(random.sample(REFUSAL_LIST,1)[0])
        ...
        with open(os.path.join(CUSTOM_DATASET_DIR, "wikitext_bio_dpo.json"), "w") as f:
            json.dump(dataset_bio_retain_dpo, f)
            
        # save to json every iter to avoid losing progress
        create_if_not_exists(CUSTOM_DATASET_DIR)
        with open(os.path.join(CUSTOM_DATASET_DIR, "wikitext_cyber_dpo.json"), "w") as f:
            json.dump(dataset_cyber_retain_dpo, f)
    
    # upload to huggingface
    final_bio = Dataset.from_dict(dataset_bio_retain_dpo)
    final_bio.push_to_hub(f"{HF_USERNAME}/wikitext_bio_dpo", private=True) 
    
    # upload to huggingface
    final_bio = Dataset.from_dict(dataset_cyber_retain_dpo)
    final_bio.push_to_hub(f"{HF_USERNAME}/wikitext_cyber_dpo", private=True) 